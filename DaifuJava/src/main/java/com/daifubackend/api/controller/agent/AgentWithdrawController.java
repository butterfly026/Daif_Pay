...
public class AgentWithdrawController extends BaseCacheController {
    ...
    @PostMapping("/list")
    public Result list(@RequestBody String requestBody, HttpServletRequest httpRequest, HttpSession session) {
        ...
        newParams.put("agent_id", agent_id);
        int page = 1;
        int page_size = 10;
...
//        return Result.success(CommonUtils.requireChannelAndCall("common", "banks", null));
    ...
    public Result doInsert(@RequestBody String requestBody, HttpServletRequest request, HttpSession session) {
        ...
        String gcode = decParams.get("code").toString();
        String gkey = userSession.getGoogleKey();
        String bank_name = decParams.get("bank_name").toString();
        String bank_open = decParams.get("bank_open").toString();
        String bank_user = decParams.get("bank_user").toString();
        String bank_card = decParams.get("bank_card").toString();
    ...
    public Result batch(@RequestBody String requestBody, HttpServletRequest httpRequest, HttpSession session) {
        ...
        JSONArray data = JSON.parseArray(decParams.get("data").toString());
